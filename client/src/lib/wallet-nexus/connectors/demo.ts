...
export class DemoMetaMaskConnector extends BaseWalletConnector {
  ...
  name = 'MetaMask (Demo)';
  icon = '🦊';
  description = 'Connect to demo MetaMask wallet for testing';
  ...
  async switchChain(walletId: string, chainId: string): Promise<void> {
    ...
    // In demo mode, chain switching is instant
  }
...
export class DemoCoinbaseConnector extends BaseWalletConnector {
  type: WalletType = 'coinbase';
  name = 'Coinbase Wallet (Demo)';
  icon = '🔵';
  description = 'Connect to demo Coinbase Wallet for testing';
  isMobileSupported = true;
  ...
  async checkConnection(): Promise<WalletInfo | null> {
    ...
    const session = JSON.parse(sessionData);
    const demoWallet = session.wallets?.find((w: WalletInfo) => w.type === 'coinbase');
    return demoWallet || null;
  }
  ...
  async connect(): Promise<WalletInfo> {
    ...
    const demoWallet = demoData.demoWallets[1];
  ...
  async getBalance(walletId: string): Promise<string> {
    return demoData.demoWallets[1].balance || '0';
  }

  async signMessage(walletId: string, message: string): Promise<string> {
    return '0xdemo_signature_coinbase_' + message.slice(0, 20) + '_' + Date.now();
  }

  async sendTransaction(walletId: string, tx: TransactionRequest): Promise<string> {
    return '0xdemo_coinbase_tx_' + Date.now() + '_' + Math.random().toString(36).substring(7);
  }
